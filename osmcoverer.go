...
func outputContainedMarkersToCsv(markers []Marker, outputDirectory string) {
	...
	for _, marker := range markers {
		...
		if len(within) > 0 {
			propName := ""
			propNameRaw, ok := marker.feature.Properties["name"]
			if !ok || propNameRaw == nil {
				propName = "unknown"
			}
			propName, ok = marker.feature.Properties["name"].(string)
			if !ok {
				propName = "unknown"
			}
			err := writer.Write([]string{propName, strconv.FormatFloat(lat, 'f', -1, 64), strconv.FormatFloat(lng, 'f', -1, 64)})
			check(err)
		}
	}
}
